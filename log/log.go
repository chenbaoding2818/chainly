...
func NewLogger(ctx context.Context, wg *sync.WaitGroup, config *config.Config) iface.ILog {
	loggerOnce.Do(func() {
		if logger == nil {
			...
			// 系统日志是否开启异步写入，异步写入时，日志会先写入缓冲区，然后异步写入到文件
			if config.LogCfg.ServerLog.AsyncEnabled {
				multiW := zerolog.MultiLevelWriter(writers...)
				bufferWriter := NewBufferWriter(ctx, wg,
					config.LogCfg.ServerLog.BufferChanelSize,
			...
			if config.LogCfg.OperationLog.RemoteEnabled {
				rh := NewRemoterHook(ctx, wg, *config.LogCfg.OperationLog, *config.MQCfg)
